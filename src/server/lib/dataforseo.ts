...
export async function fetchRankCheckSerpRaw(input: {
  ...
  console.log(
    `[rank-check-debug] keyword="${input.keyword}" target="${target}" items=${items.length} domains=${JSON.stringify(items.filter((i) => i.type === "organic").map((i) => i.domain).slice(0, 20))}`,
  );
