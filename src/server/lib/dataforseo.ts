...
// ---------------------------------------------------------------------------
// SERP Rank Check — batch version (all keywords in one request per device)
// ---------------------------------------------------------------------------

export async function fetchRankCheckBatchRaw(input: {
  ...
  if (response.status_code !== 20000) {
    throw new AppError(
      "INTERNAL_ERROR",
      response.status_message || "DataForSEO batch request failed",
    );
  }
  ...
  for (let i = 0; i < tasks.length; i++) {
    ...
    if (task.status_code !== 20000 && !isNoResults) {
      console.error(`Rank check batch failed [${kw.keyword}]: ${task.status_message}`);
      continue;
    }

    const parsedTask = successfulDataforseoTaskSchema.safeParse(task);
    if (!parsedTask.success) {
      console.error(`Rank check batch parse failed [${kw.keyword}]`);
      continue;
    }

    const billing = buildTaskBilling(parsedTask.data);
    totalCostUsd += billing.costUsd;
  ...
  return {
    data: results,
    billing: { costUsd: totalCostUsd, path: billingPath, resultCount: results.length },
  };
}
